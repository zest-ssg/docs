...
//
// Generates the English documentation feed. Rendered with no layout so the
// output is raw XML at /en/rss.xml, matching the <link rel="alternate"> tag
// that the base layout emits for English pages.
...
let posts =
    site_pages ()
    |> Array.filter (fun p -> p.url.StartsWith "/en/posts/" && p.url <> "/en/posts/")
...
sb.AppendFormat("    <atom:link href=\"{0}/en/rss.xml\" rel=\"self\" type=\"application/rss+xml\" />\n", xe siteUrl) |> ignore
